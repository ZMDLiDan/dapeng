<template>
  <div class="addAddress">
    ...
    <div class="name">
      <span>姓名：</span><input type="text" placeholder="最少两个，最多15个字" v-model="name" ref="inputName" @blur="judgeName"/>
    </div>
    <p ref="pName"></p>
    <div class="sex">
      <input class="sexInput" type="radio" name="sex" id="male" value="男" v-model="sex"/>
      <label for="male" class="sexLabel">男</label>
      <input class="sexInput" type="radio" name="sex" id="female" value="女" v-model="sex"/>
      <label for="female" class="sexLabel">女 </label>
    </div>
    <div class="tel">
      <span>电话：</span><input type="tel" placeholder="请输入手机号" v-model="tel" ref="inputTel" @blur="judgeTel"/>
    </div>
    <p ref="pTel"></p>
    <div class="address">
      <span>地区：</span>
      <div class="title">
        <div class="area" ref="province" @click="provinceSelected()" :class="Province?'':'active'">{{Province?Province:'请选择'}}</div>
        <div class="area" ref="city" @click="citySelected()" :class="City?'':'active'">{{City?City:'请选择'}}</div>
        <div class="area" ref="district" @click="districtSelected()" :class="District?'':'active'" v-show="City">{{District?District:'请选择'}}</div>
      </div>
      <ul>
        <li class="addList" v-for="(v,k) in info" :key="v.key" @click="getProvinceId(v.id, v.name, k)"
            v-show="showProvince"
            :class="v.selected ? 'active' : ''">{{v.name}}
        </li>
        <li class="addList" v-for="(v,k) in showCityList" :key="v.key" @click="getCityId(v.id, v.name, k)"
            v-show="showCity"
            :class="v.selected ? 'active' : ''">{{v.name}}
        </li>
        <li class="addList" v-for="(v,k) in showDistrictList" :key="v.key" @click="getDistrictId(v.id, v.name, k)"
            v-show="showDistrict" :class="v.selected ? 'active' : ''">{{v.name}}
        </li>
      </ul>
    </div>
    <div class="addressDetail">
      <span>地址：</span><input type="text" placeholder="请填写收获地址，5-60个字" v-model="addr" ref="inputAddr" @blur="judgeAddr"/>
    </div>
    <p ref="pAddr"></p>
    <div class="com">
      <span>设为常用地址：</span>
      <input class="comUse" type="radio" name="comUse" id="home" @click="select" value="家" v-model="street"/>
      <label for="home" class="useLabel">家</label>
      <input class="comUse" type="radio" name="comUse" id="company" @click="select" value="公司" v-model="street"/>
      <label for="company" class="useLabel">公司</label>
      <input class="comUse" type="radio" name="comUse" id="school" @click="select" value="学校" v-model="street"/>
      <label for="school" class="useLabel">学校</label>
    </div>
    <input type="hidden" name="formToken" value="${formToken}"/>
    <div class="btn" @click="hold">保存</div>
  </div>
</template>
...
<script type="text/ecmascript-6">
  export default {
    ...
    created () {
      ...
      if (this.currentAddr) {
        this.name = this.currentAddr.name;
        this.tel = this.currentAddr.telephone;
        this.sex = this.currentAddr.post;
        this.street = this.currentAddr.street;
        this.province = this.currentAddr.province;
        this.addr = this.currentAddr.address;
      }
    },
    data () {
      return {
    ...
    methods: {
      ...
      hold: function () {
        if (this.judgeName() && this.judgeTel() && this.judgeAddr()) {
          this.$http.post('http://47.95.249.186:8880/mixueshop/insertAddress', {
            province: this.$refs.province.innerHTML,
            city: this.$refs.city.innerHTML,
            district: this.$refs.district.innerHTML,
            street: this.street,
            post: this.sex,
            address: this.addr,
            name: this.name,
      ...
      judgeName: function () {
        if (this.name.length === 0) {
          this.$refs.pName.innerHTML = '* 输入姓名不能为空';
          return false;
        }
        if (this.name.length > 0 && this.name.length < 15) {
          this.$refs.pName.innerHTML = '';
          return true;
        }
        if (this.name.length > 15) {
          this.$refs.pName.innerHTML = '* 输入姓名不能超过15个字';
          return false;
        }
      },

      judgeTel: function () {
        if (!(/^1[3|4|5|7|8][0-9]\d{8,11}$/.test(this.tel))) {
          this.$refs.pTel.innerHTML = '* 输入电话格式错误';
          return false;
        } else {
          this.$refs.pTel.innerHTML = '';
          return true;
        }
      },
      ...
      judgeAddr: function () {
        if (this.addr.length === 0) {
          this.$refs.pAddr.innerHTML = '* 输入地址不能为空';
          return false;
        }
        if (this.addr.length > 0 && this.addr.length < 60) {
          this.$refs.pAddr.innerHTML = '';
          return true;
        }
        if (this.addr.length > 60) {
          this.$refs.pAddr.innerHTML = '* 输入地址不能超过60个字';
          return false;
        }
      },
      ...
      select: function () {
        var oInput = document.getElementsByClassName('comUse');
        var oLabel = document.getElementsByClassName('useLabel');
        for (var i = 0; i < oInput.length; i++) {
          if (oInput[i].checked) {
            oLabel[i].style.border = '1px solid #ff9900';
            oLabel[i].style.color = '#ff9900';
          } else {
            oLabel[i].style.border = '1px solid #999';
            oLabel[i].style.color = '#999';
          }
        }
      }
    }
